...
class TestPackaging(unittest.TestCase):
    def test_setup_script_exists_and_is_bash(self):
        ...
        self.assertIn("NOPASSWD: ${PREFIX}/scripts/nexnoc-svc", text)
        self.assertIn("Defaults:nexnoc !requiretty", text)
        self.assertNotIn("nexnoc-svc *", text)
    ...
    def test_systemd_units_bind_loopback_and_use_env_file(self):
        ...
        self.assertIn("NEXNOC_SVC_HELPER=/opt/nexnoc/scripts/nexnoc-svc", web)
        self.assertIn("NoNewPrivileges=false", web)
        self.assertNotIn("NoNewPrivileges=true", web)
        self.assertIn("NoNewPrivileges=true", poller)
        self.assertIn("NoNewPrivileges=true", trapd)
    ...
    def test_apache_template_proxies_to_loopback(self):
        ...
        self.assertIn("@@PREFIX@@", conf)
        self.assertIn("@@DATA@@", conf)
        self.assertIn("DocumentRoot @@PREFIX@@/web", conf)
        self.assertIn("ProxyPass        /api/", conf)
        self.assertNotIn("ProxyPass        / http://", conf)
        self.assertIn("Alias /tiles", conf)
        self.assertIn("Alias /uploads/pins", conf)
